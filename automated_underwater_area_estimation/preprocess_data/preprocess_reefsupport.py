...
def load_visual_mask(
    mask_image_path: str, color_threshold: int = 0
...
def process_reef_support_folder(
    ...
    if verbose:
        print(
            f"Processing {len(image_files)} images from {os.path.basename(folder_path)}..."
        )

    progress_bar = (
        tqdm(image_files, desc=f"Processing {os.path.basename(folder_path)}")
        if verbose
        else image_files
    )
...
def main():
    ...
    if args.save_metadata:
        ...
        save_processing_metadata(
            args.output_path, results, args.source_path, processing_params
        )
