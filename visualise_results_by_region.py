...
def load_detailed_results(base_path):
    ...
    print(f"Found {len(detailed_files)} detailed_results.jsonl files")
...
REGIONAL_GROUPS = {
    'tropical_atlantic': ['SEAFLOWER_BOLIVAR', 'SEAFLOWER_COURTOWN', 'SEAVIEW_ATL', 'TETES_PROVIDENCIA'],
...
print(f"\nLoaded {len(detailed_results)} individual image results")

# Create DataFrame
df = pd.DataFrame(detailed_results)

# Add regional groupings
df = add_regional_grouping(df)

# Display basic information
print("\n" + "="*80)
print("DETAILED RESULTS SUMMARY")
print("="*80)
print(f"Total image evaluations: {len(df)}")
print(f"Unique models: {df['model_name'].nunique()}")
print(f"Unique datasets: {df['dataset_name'].nunique()}")
print(f"Regions: {df['region'].unique()}")

print("\n" + "="*80)
print("DATASET DISTRIBUTION BY REGION")
print("="*80)
region_breakdown = df.groupby(['region', 'dataset_name', 'model_name']).size().reset_index(name='image_count')
for region in REGIONAL_GROUPS.keys():
    region_data = region_breakdown[region_breakdown['region'] == region]
    if len(region_data) > 0:
        print(f"\n{region.upper().replace('_', ' ')}:")
        for _, row in region_data.iterrows():
            print(f"  {row['dataset_name']} - {row['model_name']}: {row['image_count']} images")
...
print(f"Metrics to analyze: {metric_columns}")
...
for region in REGIONAL_GROUPS.keys():
    ...
    if len(region_data) > 0:
        ...
        for model in region_data['model_name'].unique():
            ...
            print(f"  {model}: {len(model_data)} images processed")
...
for region in REGIONAL_GROUPS.keys():
    region_stats = regional_df[regional_df['region'] == region]
    if len(region_stats) > 0:
        print(f"\n{region.upper().replace('_', ' ')}:")
        for _, row in region_stats.iterrows():
            print(f"  {row['model_name']}: {row['total_images']} images")
            for metric in metric_columns:
                mean_col = f'{metric}_mean'
                std_col = f'{metric}_std'
                if mean_col in row and pd.notna(row[mean_col]):
                    print(f"    {metric}: {row[mean_col]:.3f} ± {row[std_col]:.3f}")
...
for metric in metric_columns:
    ...
    for i, model in enumerate(unique_models):
        ...
        for region in unique_regions:
            ...
            region_model_data = [(m, s) for r, mo, m, s in zip(regions, models, means, stds) 
        ...
        bars = plt.bar(x + i*width, model_means, width, yerr=model_stds,
                      label=model, alpha=0.8, capsize=5,
                      color=colors[i % len(colors)],
                      error_kw={'linewidth': 2, 'capthick': 2})
        
        # Add value labels on bars
        for j, (bar, mean_val, std_val) in enumerate(zip(bars, model_means, model_stds)):
...
print(f"1. {output_file} - Regional statistics summary")
print(f"2. {detailed_output_file} - All individual results with regional groupings")
print(f"3. regional_*_comparison.png - Comparison plots for each metric")
